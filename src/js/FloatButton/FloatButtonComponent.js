...
var FloatButtomComponent = cc.Node.extend({
    ctor : function (gameId) {
        ...
        if(gameId == GameType.MiniGame_TaiXiu){
            var newCountLayer = new MiniTaiXiuNotification();
            newCountLayer.setPosition(cc.p(42,42));
    ...
    onEnter : function () {
        ...
        var thiz = this;
        thiz.img._onMouseOver = function (isOver) {
            thiz.setScale(isOver ? 1.1 : 1.0);
        };
        thiz.img.setMouseOverEnable();
    },
    onExit : function () {
        this._super();
    ...
    onTouchEnded : function (touch, event){
        var p = this.convertToNodeSpace(touch.getLocation());
        if(cc.rectContainsPoint(this.rectTouch, p)){
            var currentView = MiniGameNavigator.getRunning(this.gameId);
            if(currentView){
                currentView.closeButtonHandler();
            }
            else{
                SceneNavigator.startGame(this.gameId);
            }
        }
    }
});
