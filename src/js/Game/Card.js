...
var s_card_rect = cc.rect(3, 3, 99, 126);
...
var Card = cc.Sprite.extend({
    ctor: function (rank, suit) {
       ...
       // this.touchRect = cc.rect(0, 0, this.getContentSize().width, this.getContentSize().height);
        this.touchRect = s_card_rect;
        this.cardDistance = this.touchRect.width;
...
var CardList = cc.Node.extend({
    ...
    addCard: function (card) {
        if (!this.cardSize) {
            this.cardSize = cc.size(card.touchRect.width, card.touchRect.height);
...
var CardOnTable = cc.Node.extend({
    ...
    addNewCardList: function (cards, worldPoint) {
        //add
        var arrCard = [];
        for (var i = 0; i < cards.length; i++) {
    ...
    addCard: function (cards) {
        ...
        for (var i = 0; i < cards.length; i++) {
            var card = cards[i];
            card.setPosition(this.convertToNodeSpace(card.getPosition()));
