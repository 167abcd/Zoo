/**
 * Created by ext on 11/23/2016.
 */

var s_sfs_error_msg = s_sfs_error_msg || [];

var GameController = IController.extend({
    ctor: function () {
        this._super();
    },
    initWithView: function (view) {
        ...
        SmartfoxClient.getInstance().addListener(socket.SmartfoxClient.Login, this.onSmartfoxLoginHandler, this);
        ...
        SocketClient.getInstance().addListener("get_last_session_info", this.onGetLastSessionInfo, this);
    },
    ...
    onSmartfoxLoginHandler: function (messageType, contents) {
    ...
    onSmartfoxSocketStatus: function (type, eventName) {
        if (eventName === "LostConnection") {
            ...
            this.requestGetLastSessionInfo();
        }
    ...
    onGetLastSessionInfo: function (command, eventData) {
        cc.log(eventData);
        var hasLastSession = eventData["data"]["hasLastSession"];
        if(hasLastSession){
            var serverId = eventData["data"]["serverId"];
