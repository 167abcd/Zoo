...
var TrashCardOnTable = cc.Node.extend({
    ctor: function (width_Phom,height_Phom,typeArrange) {
        ...
        this.cardSize = cc.size(s_card_rect.width, s_card_rect.height);
    ...
    addNewCard: function (card, worldPoint) {
...
var Phom = IGameScene.extend({
    ctor: function () {
        ...
        var table_bg = new cc.Sprite("res/Texture/GameBai/gp_table.png");
        table_bg.setPosition(1000, 396);
        this.sceneLayer.addChild(table_bg);

        this.initPlayer();
        this.initButton();

        var cardList = new PhomCardList(cc.size(1300, 120));
        cardList.deckPoint = cc.p(750, 300);
        cardList.setAnchorPoint(cc.p(0.5, 0.0));
        cardList.setPosition(1000, 109);
        this.sceneLayer.addChild(cardList, 2);
        this.cardList = cardList;

        var drawDeck = new cc.Sprite("#gp_card_up.png");
        drawDeck.setPosition(1000, 400);
        this.sceneLayer.addChild(drawDeck);
        var drawDeckLabel = cc.Label.createWithBMFont(cc.res.font.Roboto_Regular_35, "52");
       ...
       //          cc.log("mobieeeeee" + i);
       //          thiz.playerView[i].setVisible(true);
       //          thiz.playerView[i].dropCards.addArrayCard(cardArr);
       //          thiz.playerView[i].trashCards.addArrayCard(cardArr2);
       //          thiz.playerView[i].setGold(10000);
       //          thiz.playerView[i].setEnable(true);
       //      };
       //     thiz.allSlot= thiz.playerView;
       //      thiz.setCardList(cardArr3);
       //     thiz.playerView[0].setUsername(PlayerMe.username);
       //     thiz.playerView[1].setUsername("player1");
       //     thiz.playerView[2].setUsername("player2");
       //     thiz.playerView[3].setUsername("player3");
       //  })));
       //
       //  setTimeout(function () {
       //      thiz.performDelegateCards("player2", "player1", 0);
       //  }, 3000);
        ...
        //         cardArr.push(card);
        //     }
        //     cardList.dealCards(cardArr, true);
        // }, 1000);
    },
    ...
    showResultDialog: function (resultData) {
        ...
        for (var i = 0; i < resultData.length; i++) {
            ...
            dialog.setGold(i, resultData[i].gold);
    ...
    performStealCard: function (stealer, stolenUser, stolenCard,
        ...
        if (stealer == PlayerMe.username) {
            ...
            cardSprite.setPosition(this.cardList.convertToNodeSpace(cardSprite.getPosition()));
    ...
    performDrawCardMe: function (cardId, groupedCard) {
        ...
        var cardSprite = new Card(card.rank, card.suit);
        var p = this.drawDeck.getParent().convertToWorldSpace(this.drawDeck.getPosition());
        cardSprite.setPosition(this.cardList.convertToNodeSpace(p));
        this.cardList.addCard(cardSprite,false);
        this.cardList.reOrder();
        this.cardList.processGroupedCard(groupedCard);
    },
    ...
    performDrawCardOther: function (username) {
        ...
        card.setScale(0.5);
        card.setPosition(this.drawDeck.getPosition());
    ...
    performDanhBaiOther: function (username, card) {
        var slot = this.getSlotByUsername(username);
        slot.trashCards.addNewCard(card, slot.getParent().convertToWorldSpace(slot.getPosition()));
        cc.log("performDanhBaiOther");
    },
    ...
    initPlayer: function () {
        ...
        playerMe.setPosition(417, 55);
        this.sceneLayer.addChild(playerMe, 1);

        playerMe.trashCards = new TrashCardOnTable(withPhom, 80,POSITION_PHOM_CENTER);
        playerMe.trashCards.setCardPosition(1000, 200);
        playerMe.dropCards = new TrashCardOnTable(withPhom,80,POSITION_PHOM_CENTER);
        playerMe.dropCards.setCardPosition(1000, 255);

        this.sceneLayer.addChild(playerMe.dropCards);
        this.sceneLayer.addChild(playerMe.trashCards);

        var player1 = new GamePlayer();
        player1.setPosition(1562, 408);
        player1.trashCards = new TrashCardOnTable(withPhom,80,POSITION_PHOM_RIGHT);
        player1.trashCards.setCardPosition(player1.width / 2 - (withPhom/2+90), -80);
        player1.dropCards = new TrashCardOnTable(withPhom,80,POSITION_PHOM_RIGHT);
        player1.dropCards.setCardPosition(player1.trashCards.x, 0);
        ...
        player2.setPosition(1000, 678);
        player2.trashCards = new TrashCardOnTable(withPhom,80,POSITION_PHOM_LEFT);
        player2.trashCards.setCardPosition(player2.width / 2 + (withPhom/2+90), -80);
        player2.dropCards = new TrashCardOnTable(withPhom,80,POSITION_PHOM_LEFT);
        player2.dropCards.setCardPosition(player2.trashCards.x, 0);
        ...
        player3.setPosition(438 , 408);
        player3.trashCards = new TrashCardOnTable(withPhom,80,POSITION_PHOM_LEFT);
        player3.trashCards.setCardPosition(player3.width / 2 +  (withPhom/2+90), -80);
        player3.dropCards = new TrashCardOnTable(withPhom,80,POSITION_PHOM_LEFT);
        player3.dropCards.setCardPosition(player3.trashCards.x, 0);
        ...
        //     this.playerView[i].trashCards.addNewCard({rank:7, suit:CardSuit.Clubs}, cc.p(0,0));
        //     this.playerView[i].trashCards.addNewCard({rank:8, suit:CardSuit.Clubs}, cc.p(0,0));
        //     this.playerView[i].trashCards.addNewCard({rank:9, suit:CardSuit.Clubs}, cc.p(0,0));
        //     this.playerView[i].trashCards.addNewCard({rank:10, suit:CardSuit.Clubs}, cc.p(0,0));
        //
        //     this.playerView[i].dropCards.addNewCard({rank:7, suit:CardSuit.Clubs}, cc.p(0,0));
        //     this.playerView[i].dropCards.addNewCard({rank:8, suit:CardSuit.Clubs}, cc.p(0,0));
        //     this.playerView[i].dropCards.addNewCard({rank:9, suit:CardSuit.Clubs}, cc.p(0,0));
        //     this.playerView[i].dropCards.addNewCard({rank:10, suit:CardSuit.Clubs}, cc.p(0,0));
        // }
    },
    initButton: function () {
        var danhbaiBt = new ccui.Button("game-danhbaiBt.png", "", "", ccui.Widget.PLIST_TEXTURE);
        danhbaiBt.setPosition(1283, 53);
        this.sceneLayer.addChild(danhbaiBt);

        var xepBaiBt = new ccui.Button("game-xepbaiBt.png", "", "", ccui.Widget.PLIST_TEXTURE);
        xepBaiBt.setPosition(1063, danhbaiBt.y);
        this.sceneLayer.addChild(xepBaiBt);

        var uBt = new ccui.Button("game-uBt.png", "", "", ccui.Widget.PLIST_TEXTURE);
        uBt.setPosition(1500, danhbaiBt.y);
        this.sceneLayer.addChild(uBt);

        var drawBt = new ccui.Button("game-bocbaiBt.png", "", "", ccui.Widget.PLIST_TEXTURE);
        drawBt.setPosition(1716, danhbaiBt.y);
        ...
        this.allButton = [danhbaiBt, uBt, xepBaiBt, drawBt, startBt, anbaiBt, habaiBt, guibaiBt];
        this.hideAllButton();
    },
