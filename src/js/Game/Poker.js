...
var PokerGamePlayer = GamePlayer.extend({
    ctor: function (playerIndex, handler) {
        ...
        lblBet.setPosition(txtBetBg.width/2, txtBetBg.height/2);
        ...
        lblHandNomarl.setColor(cc.color(255,255,255,255));
        ...
        var userLabel =  new cc.LabelTTF("whosaid",cc.res.font.Roboto_CondensedBold,24);
        this.infoLayer.addChild(userLabel);
        // userLabel.setColor(cc.color(191, 242, 255,255));
        this.goldLabel.removeFromParent();
        var goldLabel =  new cc.LabelTTF("10000",cc.res.font.Roboto_CondensedBold,24);
        // goldLabel.setPosition(thiz.avt.getPositionX(),-200);
        ...
        // thiz.avt.setVisible(false);
        this.setMoneyBet(5000000);
    },
    setIsOwner : function(isOwner){
        this.isOwnerSprite.visible = false;
    },
    getTimeCurrent:function (timeMax) {
        return this.timer.getPercentage()*timeMax/100;
    },
    ...
    setPositionInfo:function (posBase, posBet,posDel) {
        switch (posBase){
            case PK_POSITION_LEFT:
            {
                ...
                this.userLabel.setPosition(this.bgText.getPositionX()+80, this.bgText.getPositionY() + 15);
                this.goldLabel.setPosition(this.bgText.getPositionX()+80, this.bgText.getPositionY() - 15);
                break;
            }
            ...
            case PK_POSITION_TOP:
            {
                ...
                this.userLabel.setPosition(this.bgText.getPositionX(), this.bgText.getPositionY() + 15);
                this.goldLabel.setPosition(this.bgText.getPositionX(), this.bgText.getPositionY() - 15);
                break;
            }
            case PK_POSITION_BOTTOM:
            {
                this.bgText.setPosition(this.avt.getPositionX() ,this.avt.getPositionY() - 75);
                this.userLabel.setAnchorPoint(0.5,0.5);
                this.goldLabel.setAnchorPoint(0.5,0.5);
                this.userLabel.setPosition(this.bgText.getPositionX(), this.bgText.getPositionY() + 15 );
                this.goldLabel.setPosition(this.bgText.getPositionX(), this.bgText.getPositionY() - 15);
    ...
    setUserNamePoker:function () {
        ...
        this.userLabel.setColor(cc.color(255, 255, 255,255));
    },
    ...
    updateAction:function (idAction) {
        if(idAction == PK_ACTION_PLAYER_CHECK)
        {
            ...
            this.userLabel.setColor(cc.color(23, 103, 27,255));
        }
        else if(idAction == PK_ACTION_PLAYER_CALL){
            this.userLabel.setString("CALL");
            this.userLabel.setColor(cc.color(255, 152, 33,255));
        }
        else if(idAction == PK_ACTION_PLAYER_FOLD){
            this.userLabel.setString("FOLD");
            this.userLabel.setColor(cc.color(200, 44, 34,255));
        }
...
var Poker = IGameScene.extend({
    ...
    ctor: function () {
        ...
        var bgPoker = new cc.Sprite("res/Texture/GameBai/gp_table_poker.png");
       ...
       // this.bg.removeFromParent(true);
        ...
        var bg_cards = new ccui.Scale9Sprite("pk_bg_cardmain.png", cc.rect(20, 20, 4, 4));
        bg_cards.setPreferredSize(cc.size(509+ 7*5, 137 ));
        bg_cards.setPosition(cc.p(640, 385));
        this.bgPoker.addChild(bg_cards);

        var cardMix = new CardPoker(cc.size(509, 137));
        cardMix.setPosition(640 - 19, 385 );
        cardMix.visible = true;
        this.bgPoker.addChild(cardMix);
        this.cardMix = cardMix;


        // var bg_ketqua = new cc.Sprite("#pk_bg_ketqua.png");
        // bg_ketqua.setPosition(cc.p(640, 285));
        // this.bgPoker.addChild(bg_ketqua, 100);
        // cc.log("bg_ketqua "+ bg_ketqua.width);
        // var txtketqua = new cc.Sprite("#pk_ketqua0.png");
        // txtketqua.setPosition(cc.p(bg_ketqua.width/2, bg_ketqua.height/2 - 6));
        // bg_ketqua.addChild(txtketqua);
        // txtketqua.setTag(10);
        // this.bg_ketqua = bg_ketqua;


        var bg_pot = new cc.Sprite("#pk_bg_totalBet.png");
        bg_pot.setPosition(640, 497);
        ...
        var bettingPoker = new BettingZinga();
        ...
        countDown.setPosition(640,440+ pad);
        ...
        lblVcl.setColor(cc.color("#ffcb2c"));
        ...
        // this.countDown.startCountDown(5);
    },
    ...
    updateMoneyPot:function (moneyPot) {
      this.lblPot.setString(cc.Global.NumberFormat1(moneyPot));
        this.bettingPoker.setPotBetting(moneyPot);
    },
    ...
    initPlayer: function (numberSlot) {
        ...
        if(numberSlot == 5){
            ...
            var padd3 = 70;
            var distanceX1 = 224 + padd3;
            var distanceX2 = 420+ padd3;
            var distanceX3 = 420+ padd3;
            var distanceX4 = 145+ padd3;
    ...
    showSitDownDialog: function (index) {
        ...
        if(this.minBuy > PlayerMe.gold)
        {
            ...
            dialog.okButton.addClickEventListener(function () {
...
var PopupSitdown = Dialog.extend({
    ctor:function (maxBuy,minBuy, isMax) {
        ...
        var slider = new ccui.Slider("bg_ngoai.png","",ccui.Widget.PLIST_TEXTURE);
        ...
        slider.setPosition(this.getContentSize().width/2,this.getContentSize().height/2+30);
        ...
        var rightBut = new ccui.Button("pk_db_plus.png","","",ccui.Widget.PLIST_TEXTURE);
        rightBut.setPosition(this.getContentSize().width/2 + 280 ,this.getContentSize().height/2+30);
        this.addChild(rightBut);

        var leftBut = new ccui.Button("pk_db_minus.png","","",ccui.Widget.PLIST_TEXTURE);
        leftBut.setPosition(this.getContentSize().width/2-280, this.getContentSize().height/2+30);
        this.addChild(leftBut);

        var lblAuto = new cc.LabelTTF("Tự động đổi khi hết chip",cc.res.font.Roboto_Condensed,24);
        lblAuto.setColor(cc.color(239,146,14,255));
        lblAuto.setAnchorPoint(0,0.5);
        lblAuto.setPosition(278,163+30);
        this.addChild(lblAuto);

        var cbAutoBuy = new ccui.CheckBox();
        cbAutoBuy.loadTextureBackGround("pk_checkbox.png",ccui.Widget.PLIST_TEXTURE);
        cbAutoBuy.loadTextureFrontCross("pk_uncheckbox.png",ccui.Widget.PLIST_TEXTURE);
        cbAutoBuy.setSelected(true);
        cbAutoBuy.setPosition(246,163+30);
...
//popup đăt cược
var BettingZinga = cc.Node.extend({
    minBuy:0,
    maxBuy:0,
    type:PK_ACTION_PLAYER_BET,
    ctor : function () {
        ...
        var lblGold = new cc.LabelTTF("100",cc.res.font.Roboto_CondensedBold,25);
        lblGold.setColor(cc.color(251,192,1,255));
        lblGold.setPosition(bg.getContentSize().width/2,679);
        bg.addChild(lblGold);
        this.lblGold = lblGold;
        ...
        var  column_new = new cc.Sprite.create("#pk_columnnew.png");
        column_new.percent = 0;
        column_new.potMoney  = 0;
        column_new.setPosition(264,bg.getContentSize().height/2);
        bg.addChild(column_new);

        this.slider = column_new;
        var btnAllIn = new ccui.Button("pk_db_allin.png","","",ccui.Widget.PLIST_TEXTURE);
        btnAllIn.setPosition(100,568);
        btnAllIn.addClickEventListener(function () {
            column_new.percent = 100;
            var iew = Math.round(column_new.percent*20/100);
            thiz.drawSlider(iew);
        });
        bg.addChild(btnAllIn);

        var btnFull = new ccui.Button("pk_db_full.png","","",ccui.Widget.PLIST_TEXTURE);
        btnFull.setPosition(100,479);
        bg.addChild(btnFull);
        btnFull.addClickEventListener(function () {
            column_new.percent =  (column_new.potMoney -thiz.minBuy) *100 /  (thiz.maxBuy - thiz.minBuy );

            var iew = Math.round(column_new.percent*20/100);
            thiz.drawSlider(iew);
        });

        var btnHalf = new ccui.Button("pk_db_half.png","","",ccui.Widget.PLIST_TEXTURE);
        btnHalf.setPosition(100,392);
        bg.addChild(btnHalf);
        btnHalf.addClickEventListener(function () {
            column_new.percent =  (column_new.potMoney/2 -thiz.minBuy) *100 /  (thiz.maxBuy - thiz.minBuy );

            var iew = Math.round(column_new.percent*20/100);
            thiz.drawSlider(iew);
        });

        var btnPlus = new ccui.Button("pk_db_plus.png","","",ccui.Widget.PLIST_TEXTURE);
        btnPlus.setPosition(100,295);
        bg.addChild(btnPlus);
        btnPlus.addClickEventListener(function () {
            column_new.percent += 10;
            if(column_new.percent>100){
                column_new.percent = 100;
            }
            var iew = Math.round(column_new.percent*20/100);
            thiz.drawSlider(iew);
        ...
        var btnMinus = new ccui.Button("pk_db_minus.png","","",ccui.Widget.PLIST_TEXTURE);
        btnMinus.setPosition(100,208);
        bg.addChild(btnMinus);

        btnMinus.addClickEventListener(function () {
            column_new.percent -= 10;
            if (column_new.percent < 0) {
                column_new.percent = 0;
            }
            var iew = Math.round(column_new.percent * 20 / 100);
            thiz.drawSlider(iew);
        ...
        var touchRect = cc.rect(0, 0, column_new.getContentSize().width, column_new.getContentSize().height);

        for(var i  = 0; i < 21; i ++){
            var temp =  new cc.Sprite.create("#pk_c" + i+ ".png");
            temp.setPosition(48.5,237);
            column_new.addChild(temp);
            this.arrSlider.push(temp);
        }
        thiz.drawSlider (0);

        var pk_xuScale = new cc.Sprite.create("#pk_xuScale.png");
        pk_xuScale.setScale(1.2);
        pk_xuScale.setVisible(false);
        column_new.addChild(pk_xuScale);
        ...
        cc.eventManager.addListener({
            ...
            swallowTouches: true,
            onTouchBegan: function (touch, event) {
                    var p = column_new.convertToNodeSpace(touch.getLocation());
                    if (cc.rectContainsPoint(touchRect, p)) {
                        column_new.percent  = p.y*100/450;
                        ...
                        var iew = Math.round(column_new.percent*20/100);
                        thiz.drawSlider(iew);
                        pk_xuScale.setPosition(48.5,p.y+10);
                        pk_xuScale.setVisible(true);
            ...
            onTouchMoved: function (touch, event) {
                var p = column_new.convertToNodeSpace(touch.getLocation());
                // if(Math.abs(touch.getLocation().y -  touch.getPreviousLocation().y) > 3){
                //     cc.log("ccc ");
                // }
                pk_xuScale.setPosition(48.5,p.y+10);
                pk_xuScale.setVisible(true);
    ...
    drawSlider:function (index) {
        var thiz = this;
        for(var i = 0; i < this.arrSlider.length; i++){
            if(i<=index){
                this.arrSlider[i].setVisible(true);
            }else {
                this.arrSlider[i].setVisible(false);
            }
        }
        thiz.gold =  Math.floor(this.slider.percent*(thiz.maxBuy - thiz.minBuy)/100) + thiz.minBuy;
        thiz.lblGold.setString(cc.Global.NumberFormat1(thiz.gold));
        thiz.allIn.setVisible((thiz.slider.percent<100)?false:true);
        thiz.type = (thiz.slider.percent<100)?thiz.typeOld:PK_ACTION_PLAYER_ALL_IN;
        thiz.lblGold.setVisible(!thiz.allIn.isVisible());
    ...
    setPotBetting:function (money) {
        this.slider.potMoney = money;
    },
