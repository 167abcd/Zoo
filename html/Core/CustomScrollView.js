/**
 * Created by cocos2d on 4/13/2017.
 */

var newui = newui || {};
newui._addMouseScrollEvent = function (view, cb) {
    if(newui.s_customScrollViewMouseEvent === undefined){
        var listener = cc.EventListener.create({
            ...
            onMouseScroll: function (event) {
                ...
                cc.eventManager.dispatchCustomEvent("customMouseScroll", obj);
                return false;
            }
        });
        cc.eventManager.addListener(listener, 1);
        newui.s_customScrollViewMouseEvent = listener;
    }

    cc.eventManager.addListener({
        event: cc.EventListener.CUSTOM,
        eventName : "customMouseScroll",
        callback : function (e) {
            if(e._userData["swallowEvent"] === false){
                var event = e._userData["event"];
                if(cb(event)){
                    e._userData["swallowEvent"] = true;
                }
            }
        }
    }, view);
...
newui.CustomScrollView = ccui.ScrollView.extend({
    ...
    onEnter : function () {
        ...
        var thiz = this;
        newui._addMouseScrollEvent(this, function (event) {
