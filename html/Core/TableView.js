...
newui.TableView = newui.CustomScrollView.extend({
    ctor : function (size, columnCount) {
        ...
        this._customRefreshControl = null;
        this._onBeforeRefreshView = null;
        this._onAfterRefeshView = null;
    },
    ...
    setCustomRefreshControl : function (cb) {
        this._customRefreshControl = cb;
    },

    setBeforeRefreshViewEvent : function (cb) {
        this._onBeforeRefreshView = cb;
    },

    setAfterRefeshViewEvent : function (cb) {
        this._onAfterRefeshView = cb;
    },
    ...
    forceRefreshView : function () {
        if(this._onBeforeRefreshView){
            this._onBeforeRefreshView();
        }
        if(this._customRefreshControl){
            this._refreshViewCustomHandler();
        }
        else{
        ...
        this._refreshView = false;
        if(this._onAfterRefeshView){
            this._onAfterRefeshView();
        }
    },
    ...
    _refreshViewCustomHandler : function () {
        var containerWidth = this.getContentSize().width;
        var containerHeight = this.getContentSize().height;
        if(this._allItems.length > 0){
            for(var i=0; i<this._allItems.length;i++){
                this._allItems[i].setAnchorPoint(cc.p(0.5, 0.5));
                this._customRefreshControl(this._allItems[i], i);
            }


            if(this._direction === ccui.ScrollView.DIR_VERTICAL){
                var topItem = this._allItems[0];
                for(var i=1; i<this._allItems.length;i++){
                    if(this._allItems[i].y > topItem.y){
                        topItem = this._allItems[i];
                    }
                }
                containerHeight = topItem.y + topItem.height/2 + this._marginTop;
            }
            else {
                var rightItem = this._allItems[0];
                for(var i=1; i<this._allItems.length;i++){
                    if(this._allItems[i].x > rightItem.x){
                        rightItem = this._allItems[i];
                    }
                }
                containerWidth = rightItem.x + rightItem.width/2 + this._marginRight;
            }
        }
